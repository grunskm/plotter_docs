...
long width = 30000; //X-axis (rail)
int height = 30000; //Y-axis (board)
  ...
  for(int q=0;q<y_num-2;q++){
    for(int e=0;e<x_num-2;e++){
      if(printAborted==false){
        int posX = e*space+(space/2);
        int posY = q*space+(space/2);
        perimeter(posX,posY,space,space);
        r_angle += r_step;
        cube3D(posX,posY,wid,r_angle);
      }
    }
  }
...
void cube3D(int X,int Y,int W,float N){
  int dim = 3;
  int n = 0;
