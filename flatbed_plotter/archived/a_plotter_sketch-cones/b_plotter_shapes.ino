...
void ellipse(int X, int Y, int W, int H){
  ...
  float a = TWO_PI/steps;
  for(int g=0;g<steps;g++){
    float cir_x = X+sin((a*g)-(TWO_PI/4))*W;
    float cir_y = Y+cos((a*g)-(TWO_PI/4))*H;
...
void randomSquiggleSmooth(int x_min,int x_max,int y_min, int y_max, int R){
  ...
  for(int j=0;j<1;j++){
    int x_1 = x;
    int y_1 = y;
    int seed = random(0,3000);
    for(int m=0; m<R; m++){
      if(m==1){
        toggle_pen();
      }
      int n = seed+m;
      float wiggle1 = (sin(n*0.01)+sin(n*0.012))*3.5/10;
      float wiggle2 = (sin(n*0.2)+sin(n*0.3))*1.5/10;
      a += (wiggle1+wiggle2)*deg;
      int x_2 = x_1+sin(a)*r;
      int y_2 = y_1+cos(a)*r;
      plotLineCont(x_1,y_1+(j*500),x_2,y_2+(j*500));
      x_1 = x_2;
      y_1 = y_2;
    }
    toggle_pen();
...
void randomSquiggle(int x_min,int x_max,int y_min, int y_max, int R){
  float deg = TWO_PI/360;
  float a = deg*random(0,360);
  int r = 50;
