...
void setup() {
 // Serial.begin(9600);
  ...
  while(off<h/2 && printAborted==false){
    count=0;
    off+=space;
    arcDashed(margin+w/2,margin+h/2,off,off,TWO_PI,TWO_PI*0.5);
    lineDashed(margin+w/2,margin+h/2-off,margin+w/2-off,margin+h/2-off);
    lineDashed(margin+w/2-off,margin+h/2-off,margin+w/2-off,margin+h);
  }
