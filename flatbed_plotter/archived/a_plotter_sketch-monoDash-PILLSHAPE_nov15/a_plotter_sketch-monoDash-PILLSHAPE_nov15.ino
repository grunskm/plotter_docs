...
void setup() {
 // Serial.begin(9600);
  ...
  int innerh = height*0.33;
  int inset = (width-height)/2;
  
  int off = 0;
  int space = 50;

  while(off<innerh-50 && printAborted==false){
    //count=0;
    off+=space;
    
    arcDashed( inset+height*0.66, height*0.5, off, off, TWO_PI, TWO_PI*0.5,1);
    lineDashed( inset+height*0.66, height*0.5-off, inset+height*0.66-off, height*0.5-off);
    lineDashed( inset+height*0.66-off, height*0.5-off, inset+height*0.66-off, height*0.5+(innerh-off));
    lineDashed( inset+height*0.66-off, height*0.5+(innerh-off), inset+height*0.33, height*0.5+(innerh-off));
    arcDashed( inset+height*0.33, height*0.5, innerh-off, innerh-off, TWO_PI,TWO_PI*0.5,-1);
  }
