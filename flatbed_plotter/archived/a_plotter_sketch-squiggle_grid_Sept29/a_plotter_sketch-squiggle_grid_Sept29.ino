    ...
    for(int e=0;e<x_num-2;e++){
     ...
     for(int h=0;h<100;h++){
      if(printAborted==false){
        ...
        float sz = wid;//*(sin((h+seed)*s2)+cos((h+seed)*s5)+sin((h+seed)*s3)+cos((h+seed)*s8))/4.00;
        
        float s = 1/(1+sz*scale);
        
        float sx = sin(h*(TWO_PI/100))*wid;//*(sin((h+seed)*s1)+cos((h+seed)*s2)+sin((h+seed)*s3)+cos((h+seed)*s4))/4.00*s;
        float sy = cos(h*(TWO_PI/100))*wid;//*(cos((h+seed)*s5)+sin((h+seed)*s6)+cos((h+seed)*s7)+sin((h+seed)*s8))/4.00*s;
