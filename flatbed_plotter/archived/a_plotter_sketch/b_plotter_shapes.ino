...
void grass(int X, int Y, int MIN, int MAX){
  ...
  int servo_a = 180;
 // int speed1 = 200;
  int x = X+random(-100,100);
  int y = Y+random(-100,100);
  moveToPointAcc(x,y);
  toggle_pen();
  for(int g=0;g<len;g++){
    int xx = x+pow(g,2)/4000;
    int yy = y-g;
...
void perimeter(){
  ...
  moveToPointAccDraw(0,height);
  moveToPointAccDraw(width,height);
  moveToPointAccDraw(width,0);
  moveToPointAccDraw(0,0);
 // toggle_pen();  
  moveToPointAccDraw(0,height);
  moveToPointAccDraw(width,height);
